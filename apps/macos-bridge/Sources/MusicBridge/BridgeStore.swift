        ...
        monitor = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                do {
                    ...
                    failures = 0
                    if playback.fingerprint != fingerprint { try await CloudClient.sendPlayback(playback, config: config); fingerprint = playback.fingerprint }
                    self?.status = "WiiM conectado · reproducción sincronizada"
                } catch {
                    failures += 1; self?.status = "No se pudo consultar el WiiM; reintentando…"
                    if failures >= 3 { host = nil; failures = 0 }
                }
