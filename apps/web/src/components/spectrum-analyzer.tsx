...
type MarqueeSpeed = "relaxed" | "fast" | "turbo" | "ultra";
...
const marqueeSpeeds: Record<MarqueeSpeed, { label: string; divisor: number }> = { relaxed: { label: "1×", divisor: 45 }, fast: { label: "2×", divisor: 24 }, turbo: { label: "3×", divisor: 15 }, ultra: { label: "4×", divisor: 11 } };
...
export const SpectrumAnalyzer = ({ track, album, artist }: { track: string; album: string; artist: string }): React.JSX.Element => {
  ...
  const [marqueeSize, setMarqueeSize] = useState<MarqueeSize>("large");
  ...
  useEffect(() => () => stop(), [stop]);
  useEffect(() => {
    const updateFullscreen = (): void => setIsFullscreen(document.fullscreenElement === panel.current);
  ...
  useEffect(() => {
    ...
    const draw = (): void => {
      ...
      if (visualizer === "scope") {
        ...
        const text = ledText(`NOW PLAYING — ${track} · ${album} · ${artist}`); const size = marqueeSizes[marqueeSize]; const cell = Math.max(5, Math.min(size.maxCell, height * size.proportion / 7)); const cellGap = Math.max(1, cell * .16); const messageWidth = Array.from(text).reduce((total, character) => total + ((ledGlyphs[character] ?? ledGlyphs["?"])[0].length + 1) * cell, 0); const gap = height * .42; const offset = performance.now() / marqueeSpeeds[marqueeSpeed].divisor % (messageWidth + gap); const start = width - offset; const top = (height - cell * 7) / 2;
  ...
  }, [open, theme, visualizer, marqueeColor, marqueeSpeed, marqueeSize, isFullscreen]);

  const toggleFullscreen = async (): Promise<void> => {
    if (isTheaterMode) { setIsTheaterMode(false); return; }
    if (document.fullscreenElement === panel.current) { await document.exitFullscreen(); return; }
    try {
      if (!panel.current?.requestFullscreen) throw new Error("Fullscreen no disponible");
