...
type View = "album" | "archive" | "credits" | "artist";
const views: Array<{ id: View; label: string }> = [{ id: "album", label: "El disco" }, { id: "archive", label: "Archivo" }, { id: "credits", label: "Créditos" }, { id: "artist", label: "Artista" }];
const demo: AgentPlaybackPayload = { agentVersion: "demo", deviceId: "living-room", playbackProvider: "qobuz", source: "Qobuz", artist: { name: "The Beatles" }, album: { title: "Abbey Road", artworkUrl: "https://coverartarchive.org/release-group/8d6c0f78-5ea7-4af2-adaf-23966d13ef7e/front-500" }, track: { title: "Something", durationMs: 182000, positionMs: 61000 }, playback: { state: "playing" } };
const demoAlbum: AlbumCompanionData = { id: "demo", artistId: "demo", title: "Abbey Road", artist: "The Beatles", year: 1969, label: "Apple Records", genres: [], tags: [], enrichment: [], tracks: [], editions: [], credits: [], artwork: [] };
...
export const NowPlayingExperience = (): React.JSX.Element => {
  const [playback, setPlayback] = useState(demo);
  const [album, setAlbum] = useState<AlbumCompanionData>(demoAlbum);
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      ...
      try {
        ...
        if (requestId !== latestRequest.current || !data.playback) return;
        ...
        if (lastPlaybackKey.current && lastPlaybackKey.current !== playbackKey) { fastPollingUntil.current = Date.now() + 8_000; setSelectedEditionId(undefined); setView("album"); }
  ...
  const discogsEditions = album.editions.filter((item) => item.discogsReleaseId);
  const selectedEdition = discogsEditions.find((edition) => edition.id === selectedEditionId) ?? discogsEditions[0];
  const editionCredits = selectedEdition ? album.credits.filter((credit) => credit.releaseId === selectedEdition.id) : [];
  ...
  const editionPicker = discogsEditions.length ? <div className="edition-picker">{discogsEditions.map((edition) => <button className={edition.id === selectedEdition?.id ? "selected" : ""} key={edition.id} onClick={() => setSelectedEditionId(edition.id)}>{[edition.country, edition.year, edition.formatDescription ?? edition.format].filter(Boolean).join(" · ") || "Edición Discogs"}</button>)}</div> : null;
  ...
  const creditsView = <section className="detail-section credits-panel"><div className="content-heading"><p className="section-label">Créditos</p><p>Personas y oficios asociados a una edición concreta.</p></div>{editionPicker}{editionCredits.length ? <div className="credits-list">{editionCredits.map((credit) => <p className="credit" key={credit.id}><strong>{credit.personName}</strong><span>{credit.role}</span></p>)}</div> : isResolving ? editorialLoading : <p className="empty-state">Los créditos detallados aparecen cuando haya una edición identificada.</p>}</section>;
